...
page, as examples of the app running. Not in the README: it carries no
screenshots at all since 2.9.91, and a test keeps it that way. Nowhere else — not as page
...
## Stopping point, 22 Sept (build 2.9.102)
...
- The landing page's Dip/Dive/Multi-Dip example now uses the new VIAL
  chooser capture instead of Maciann, cut cleanly from chroma green (2.9.97)
...
- Landing sections now follow the product journey, and the VIAL progress
  capture uses Joseph's corrected replacement image (2.9.82)
...
- Demo Dives now routes Crate, Multi-Dip and the library scan into their
  staged data. Demo Mixes stages the real mix of date, album, discovery,
  random and artist-based recipes instead of an artist-only grid (2.9.65)
...
~~**"All" is still offered on the standard mix dialog.**~~ *Removed in
