...
import * as demo from "./demo.js?v=2.9.101";
...
function openIntentModal(artistName, { force = false, chroma = false } = {}) {
  ...
  // Multi-Dip starts from whoever you just searched, since that's almost
...
function renderScrubForm() {
  setTitle("DeepDive · Duplicate scan");
  setActiveTab("scrub");
  root.innerHTML = `
    <div class="card">
      <h1>Find duplicate songs</h1>
      <p class="muted">Checks each artist's releases for other Spotify copies of songs already in your Liked Songs. Like those copies so future dives know they are not new. It never builds a playlist, but a large library can take hours.</p>
      <div class="actions" style="margin-top:0;">
        <button class="btn btn-primary" id="scrub-go">Find duplicate copies</button>
...
async function startScrub() {
  ...
  if (blockedByRateLimit()) return;
  showDiveScreen("Finding duplicate copies…", () => { scrubCancel.cancelled = true; });
...
function renderScrubResults(r) {
  setTitle("DeepDive · Duplicate results");
  const dups = r.duplicate_candidates || [];
  root.innerHTML = `
    <div class="card">
      <h1>Duplicate scan ${r.artists_scanned < r.artists_total ? "cancelled" : "complete"}</h1>
      <p class="muted">Checked ${r.artists_scanned ?? "your"} of ${r.artists_total ?? "all"} artists · ${dups.length} duplicate cop${dups.length === 1 ? "y" : "ies"} found</p>
      ${dups.length ? `
        <div class="crate-header"><span class="label">Copies to mark as liked</span></div>
        <p class="crate-note">Each one matches a recording already in your Liked Songs. Like the selected copies so they stop appearing as new in future dives.</p>
        <div id="dup-list">${dups.map((d) => dupRow(d)).join("")}</div>
        <div class="actions"><button class="btn btn-primary" id="scrub-like">Like selected copies</button><button class="btn btn-ghost" data-home>Back to Dives</button></div>
        <div class="flash hidden" id="result-msg" style="margin-top:18px;"></div>
      ` : `<p class="empty-note">No duplicate recordings found.</p><div class="actions"><button class="btn btn-ghost" data-home>Back to Dives</button></div>`}
    </div>`;
  root.querySelector("[data-home]")?.addEventListener("click", () => renderDives());

  const likeBtn = document.getElementById("scrub-like");
  if (likeBtn) likeBtn.addEventListener("click", async () => {
    const msg = document.getElementById("result-msg");
    const ids = Array.from(document.querySelectorAll("[data-dup]:checked")).map((c) => c.dataset.dup);
    if (!ids.length) {
      msg.textContent = "Select at least one duplicate copy to like.";
      msg.classList.remove("hidden", "error");
      return;
    }
    likeBtn.disabled = true;
    try {
      await client.likeTracks(ids);
      await libraryCache.clear();
      history.recordAction({ type: "like", label: `Liked ${ids.length} duplicate cop${ids.length === 1 ? "y" : "ies"}`, trackIds: ids });
      showActionResult({
        headline: "Duplicate copies liked",
        detail: `${ids.length} cop${ids.length === 1 ? "y" : "ies"} will now count as already liked in future dives.`,
      });
    } catch (e) {
